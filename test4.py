...
AND_GATE_IMAGE = "/Users/madhusudhan/Desktop/Static-SignOff-Simulator/AND3.png" 
...
def var_labeler(inputs_sorted, all_nodes_sorted):
    """Label all nodes with A,B,C,... in topological order"""
    labels = {}
    letters = [chr(ord('A') + i) for i in range(26)]
    for idx, nid in enumerate(all_nodes_sorted):
        labels[nid] = letters[idx] if idx < len(letters) else f"N{nid}"
    return labels
...
def render_block(defs, idx):
    ...
    layers, maxlvl, lvl_map = levels(parents)
    
    # Get all nodes sorted by their level (topological order)
    all_nodes_sorted = []
    for d in sorted(layers.keys()):
        all_nodes_sorted.extend(sorted(layers[d]))
    
    node_labels = var_labeler(None, all_nodes_sorted)
        ...
        with g.subgraph(name=f"cluster_level_{d}") as s:
            ...
            for n in sorted(layers.get(d, [])):
                ...
                if k == 0:
                    ...
                    s.node(f"n{n}", label=node_labels[n], shape="triangle", orientation="90",
                    ...
                    # AND gate (image) with label overlaid - Box with image and centered label
                    s.node(f"n{n}", label=node_labels[n], shape="box", 
